...
<div class="container">
	<h1>글수정</h1>
	<form action="${contextPath}/game/modify" method="post" enctype="multipart/form-data">
		<input type="hidden" name="gno" value="${b.gno}">
		<table class="table">
			<tr>
				<td>제목</td>
				<td>
			...
			<tr>
				<td>작성자</td>
				<td>
					<input type="text" class="form-control" name="volSize" value="${b.volSize}">
				</td>
			</tr>
			<tr>
				<td>작성자</td>
				<td>
					<input type="text" class="form-control" name="developer" value="${b.developer}">
				</td>
			</tr>
			<tr>
				<td>첨부파일</td>
				<td>
					...
					<c:if test="${not empty b.fileName}">
						...
						<c:if test="${b.fileType eq 'IMAGE'}">
							<a href="${contextPath}/download?fileName=${b.gno}/${b.fileName}">
								<img src="${contextPath}/gameImgDisplay?fileName=${b.gno}/${b.fileName}">
							</a>
						</c:if>
						<c:if test="${b.fileType eq 'OTHER'}">
							<a href="${contextPath}/download?fileName=${g.bno}/${b.fileName}">
								${b.fileName}
							</a>
						</c:if>
					</c:if>
